...
export class HeuristicReferenceMatcher {
    ...
    public match(
    ...
    ): Array<{ start_in_seq: number; end_in_seq: number; start_in_ref: number; end_in_ref: number }> {
        const results = [];
        ...
        for (let i = 0; i < sequences.length; i++) {
            ...
            const refObj = this.referenceAlleles[allele];
            if (!refObj) {
                console.error(`Reference allele not found for ${allele}`);
                continue;
            }
            const refSeq = refObj.sequence;
