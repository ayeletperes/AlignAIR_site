...
export const AlignmentBrowserHeavy: React.FC<AlignmentBrowserProps> = ({ results, referenceAlleles }) => {
    ...
    const jAAIndex: number = Math.floor(results.j_sequence_start / 3);
    ...
    const splitD = splitSequence(results.sequence.slice(results.v_sequence_end,results.j_sequence_start), maxCharsPerRow)
    const splitDAA = splitSequence(sequenceD, maxCharsPerRow/3)
    const splitJ = splitSequence(results.sequence.slice(results.j_sequence_start), maxCharsPerRow)
    const splitJAA = splitSequence(sequenceJ, maxCharsPerRow/3)

    const np1 = results.d_sequence_start - (results.v_sequence_end);
    const np2 = results.j_sequence_start - (results.d_sequence_end);
    console.log(splitD, sequenceD)
    console.log(splitD[0].length, sequenceD.length)
    const reamningV = results.v_sequence_end-sequenceV.length*3;
    const reamningD = results.j_sequence_start-(sequenceV.length+sequenceD.length)*3;
    ...
    useEffect(() => {
        ...
        setMismatchD(GetSequenceMismatchIdx(results.sequence.slice(results.d_sequence_start,results.d_sequence_end), selectedSequenceD, maxCharsPerRow));
