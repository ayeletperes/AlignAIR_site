...
import {anchorLightJ} from './reference'
...
export const AlignmentBrowserLight: React.FC<AlignmentBrowserProps> = ({ results, referenceAlleles }) => {
    ...
    if(gappNotes===''){
      ...
      const anchor = results.j_call[0]?anchorLightJ[results.j_call[0]]:null;
      if(anchor){
    ...
    useEffect(() => {
      setSelectedSequenceV(
        referenceAlleles['v_call'][results.v_call[0]].slice(0, results.v_germline_end)
      );
  ...
  if(selectedSequenceV && selectedSequenceJ && splitedGVAA.length>0 && splitedGJAA.length>0 && sequenceJ!="" && sequenceV!="" && sequence!="" && sequenceAA!="" && germlineAA!="" && splitVAA.length>0 && splitJAA.length>0){  
