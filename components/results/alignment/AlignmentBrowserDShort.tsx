...
export const AlignmentBrowserHeavyDshort: React.FC<AlignmentBrowserProps> = ({ results, referenceAlleles }) => {
  ...
  useEffect(() => {
    setSelectedSequenceV(
      referenceAlleles.invertedV[results.v_call[0]].sequence.slice(results.v_germline_start, results.v_germline_end)
    );
        ...
        <div className="subgrid-col2" style={{gridColumn:2, height:`${splitD.length*75}"px"`,gridTemplateRows:`repeat(${splitD.length},"20px")`, gridTemplateColumns:`repeat(${maxCharsPerRow},"15px")`}}>
          {splitD.map((chunk, index) => (
            ...
            {splitD.length === 1 ? (
                    ...
                    <AlignedBlock sequence={chunk} regions={Dregions ? Dregions[index] : Dregions} aasequence={splitDAA[index]} germline={splitedSequenceD[index]} aagermline={splitedGDAA[index]} mismatch={mismatchD[index]} np1={np1} np2={splitedNP2[index]} remainingNucPrev={remainingV} remainingNucCur={remainingD}/>
