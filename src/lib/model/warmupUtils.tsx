...
export async function warmUpSingleModel(
  ...
  try {
    ...
    logger.info(`Model ${chainConfig.name} warmed up successfully in ${warmupTime.toFixed(2)}ms`);
...
export async function warmUpModels(
  ...
  logger.info(`Starting warmup for ${chainConfigs.length} models ${parallel ? 'in parallel' : 'sequentially'}...`);
  ...
  logger.info(`Warmup complete: ${successful}/${chainConfigs.length} models warmed up successfully`);
