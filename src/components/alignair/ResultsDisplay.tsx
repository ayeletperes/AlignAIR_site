...
export function ResultsDisplay() {
  ...
  // State for run metadata
  const [runMetadata, setRunMetadata] = useState<any>(null);
  const [metadataDownloadOptions, setMetadataDownloadOptions] = useState<any[]>([]);

  // Generate run metadata when results change
  useEffect(() => {
    const generateMetadata = async () => {
      if (latestResult && params && input) {
        try {
          ...
          setMetadataDownloadOptions(getMetadataDownloadOptions(metadata));
        } catch (error) {
          console.error('Failed to generate run metadata:', error);
          setRunMetadata(null);
          setMetadataDownloadOptions([]);
        }
      }
    };

    generateMetadata();
  }, [latestResult, params, input]);
            ...
            {latestResult && (
              <div className="mt-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                ...
                <div className="flex space-x-3">
                  {metadataDownloadOptions.map((option) => (
                    <button
                      key={option.format}
