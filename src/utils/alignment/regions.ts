...
function findRegionIndicesInChunks(gappedAaRegions: string[], ungappedAaChunks: string[], ungappedNtChunks: string[]): { region: string, ntIndices: [number, number] }[] {
  ...
  type ChunkRegions = {
    ntChunk: string,
    regions: { region: string, ntIndices: [number, number] }[]
  };
  
  function findRegionIndicesForNtChunks(
    gappedAaRegions: { [key: string]: string },
    ungappedAaChunks: string[],
    ungappedNtChunks: string[]
  ): ChunkRegions[] {
  
    const aaSeq = ungappedAaChunks.join('');
    const ntSeq = ungappedNtChunks.join('');
  
    // 1) Build ordered AA spans per region by walking cumulatively.
    // We strip '.' from each region string to get its ungapped length.
    const regionSpans: { region: string, aaStart: number, aaEnd: number, ntStart: number, ntEnd: number }[] = [];
    let aaCursor = 0;
  
    for (const [region, seq] of Object.entries(gappedAaRegions)) {
      const lenAA = seq.replace(/\./g, '').length;
      if (lenAA === 0) continue; // skip empty regions
  
      const aaStart = aaCursor;
      const aaEnd = aaStart + lenAA;
    ...
    if (aaCursor !== aaSeq.length) {
      logger.debug(`AA length mismatch. Regions cover ${aaCursor}, AA sequence has ${aaSeq.length}.`);
    }
    if (ntSeq.length !== aaSeq.length * 3) {
      logger.debug(`NT length mismatch. NT=${ntSeq.length}, AA*3=${aaSeq.length * 3}.`);
    }
  
    // 2) Walk NT chunks in order, intersect with region NT spans.
    const result: ChunkRegions[] = [];
    let ntCursor = 0;
  
    for (const ntChunk of ungappedNtChunks) {
      const startNt = ntCursor;
      const endNt = startNt + ntChunk.length;
  
      const chunkRegions: { region: string, ntIndices: [number, number] }[] = [];
  
      for (const span of regionSpans) {
        const s = Math.max(startNt, span.ntStart);
        const e = Math.min(endNt, span.ntEnd);
        if (e > s) {
          chunkRegions.push({ region: span.region, ntIndices: [s, e] });
        }
      }
